...
//!
//! Dispatched from upstream `core/src/avm1/globals/asnative.rs` (hook H1).
//! Every call is logged under target `chumby_host` — this is the project's
//! runtime visibility layer (stock Ruffle gives no signal for natives,
//! gap analysis G2). Index/name table: environment contract 03 §1.
...
fn dispatch<'gc>(
    ...
    let result = match index {
        // --- Category 4: persistence (virtual rootfs) ---
        ...
        // _getDirectoryEntry(obj, path, index): fills obj and returns a
        // status code. Object-filling lands with the USB-music step; until
        ...
        // _md5Sum: real digest deferred; constant marker keeps boot paths
        // alive (only equality-with-itself matters in observed call sites).
        162 => {
...
/// Canonical wrapper names from the environment contract (03 §1).
/// Used for logging and for `ChumbyHost::native` dispatch-by-name.
