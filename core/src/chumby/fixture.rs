...
//!
//! Layout (chumby-pi project, `docs/design/chumby-host.md` §5):
//! ```text
//! fixtures/
...
pub struct FixtureHost {
    ...
    /// Master/slave variable exchange (5,80/5,81). `_chumby_widget_done`
    /// defaults to "true" so intro/widget handoffs never hang (03 §1a).
...
impl ChumbyHost for FixtureHost {
    fn native(&self, _index: u16, name: &str, args: &[HostValue]) -> HostValue {
        match name {
            ...
            "_dcVolts" => HostValue::Number(12.0),
            "_powerSource" => HostValue::Number(1.0), // external power
            "_headphonesIn" => HostValue::Number(0.0),
            ...
            // btplay daemon lifecycle — not applicable to our mpv model.
            "_startAudioPlayer" | "_restartAudioPlayer" => HostValue::Undefined,

            // --- Volume: intercept _setSystemVolume/_setSystemMute for live
            //     mpv control and persistence across restarts. ---
            "_setSystemVolume" => {
                if let Some(HostValue::Number(n)) = args.first() {
            ...
            // Stateful get/set pairs share a name-keyed store. Setters are
            // named "_setX", getters "_getX" — store under the X part.
...
fn default_for_getter(name: &str) -> HostValue {
    match name {
        "_getSystemVolume" => HostValue::Number(60.0),
        "_getSystemBalance" => HostValue::Number(0.0),
        "_getSystemMute" | "_getSpeakerMute" | "_getLCDMute" => HostValue::Number(0.0),
        "_getTouchClick" => HostValue::Number(0.0),
        "_getLCDBrightness" => HostValue::Number(65536.0),
        _ => HostValue::Undefined,
    }
}
