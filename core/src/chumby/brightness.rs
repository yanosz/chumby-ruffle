...
//!   never assumed — times `brightness_cap` (config.rs), the percent of
//!   that range the panel's 100% reaches.
...
pub struct Backlight {
    ...
    /// `max_brightness × cap`, i.e. what the panel's 100% reaches. Not the
    /// device maximum, which nothing here writes once a cap is configured.
    max: f64,
...
impl Backlight {
    pub fn detect(cap_percent: f64) -> Option<Backlight> {
        Self::detect_in(Path::new("/sys/class/backlight"), cap_percent)
    }
    ...
    pub(crate) fn detect_in(class_dir: &Path, cap_percent: f64) -> Option<Backlight> {
        ...
        let cap_percent = cap_percent.clamp(0.0, 100.0);
        let capped = max as f64 * cap_percent / 100.0;
        if capped < 1.0 {
            tracing::warn!(target: "chumby_host",
                "brightness_cap {cap_percent}% of {max} rounds below one step — \
                 the backlight can then only switch, not dim");
        }
        tracing::info!(target: "chumby_host",
            "backlight {} (max_brightness={max}, cap {cap_percent}% → {capped:.0})",
            dir.display());
        Some(Backlight {
    ...
    /// Panel space 0–65535 → device space 0–max×cap. Linear; 0 stays 0 (the
    ...
    pub fn set_raw(&self, value: f64) {
        ...
        let mut scaled = (value / 65535.0 * self.max).round() as u32;
        if value > 0.0 {
            scaled = scaled.max(1);
        }
...
mod tests {
    ...
    fn test_detection_prefers_rpi_then_lone_device() {
        let dir = tmp("detect");
        assert!(Backlight::detect_in(&dir, 100.0).is_none(), "empty class dir");

        fake_device(&dir, "10-0045", "255");
        let b = Backlight::detect_in(&dir, 100.0).expect("lone device");
        assert!(b.brightness_file.ends_with("10-0045/brightness"));
        assert_eq!(b.max, 255.0);

        fake_device(&dir, "rpi_backlight", "255");
        let b = Backlight::detect_in(&dir, 100.0).expect("rpi_backlight");
        ...
        assert!(Backlight::detect_in(&dir, 100.0).is_none(), "two devices, neither rpi");
    ...
    fn test_set_raw_scales_to_device_max() {
        let dir = tmp("scale");
        fake_device(&dir, "rpi_backlight", "255");
        let b = Backlight::detect_in(&dir, 100.0).unwrap();
        let read = || -> u32 {
    ...
    fn test_bad_max_brightness_rejected() {
        ...
        assert!(Backlight::detect_in(&dir, 100.0).is_none());
        std::fs::write(dir.join("rpi_backlight/max_brightness"), "junk").unwrap();
        assert!(Backlight::detect_in(&dir, 100.0).is_none());
        std::fs::remove_dir_all(&dir).ok();
    }

    /// `brightness_cap`: the panel's 100% reaches that percent of the
    /// device max, and the slider's whole travel spreads over it — which is
    /// the point, fine steps down low. The ≥1 floor still holds, so no
    /// slider position the panel means as "on" turns the screen off.
    #[test]
    fn test_set_raw_honours_the_cap() {
        let dir = tmp("cap");
        fake_device(&dir, "rpi_backlight", "255");
        let b = Backlight::detect_in(&dir, 25.0).unwrap();
        let read = || -> u32 {
            std::fs::read_to_string(dir.join("rpi_backlight/brightness"))
                .unwrap()
                .parse()
        ...
        b.set_raw(65535.0); // panel 100% → a quarter of the range
        assert_eq!(read(), 64);
        b.set_raw(32767.0); // panel 50%
        assert_eq!(read(), 32);
        b.set_raw(983.0); // panel 1.5, the box's night setting
        assert_eq!(read(), 1);
        b.set_raw(1.0); // dimmest non-zero still on
        assert_eq!(read(), 1);
        b.set_raw(0.0); // screen off
        assert_eq!(read(), 0);

        // A cap so small the range collapses: on or off, nothing between.
        fake_device(&dir, "rpi_backlight", "3");
        let b = Backlight::detect_in(&dir, 25.0).unwrap();
        b.set_raw(65535.0);
        assert_eq!(read(), 1);
