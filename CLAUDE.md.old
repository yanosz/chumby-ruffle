...
- **`chumby` carries exactly two commits on top of upstream**, and always
  rebases onto ruffle upstream — never merges. The first commit adds files
  upstream does not have; the second modifies or deletes files it does. The
  test is mechanical: a path absent from `upstream/master` belongs in the
  first commit, a path present there in the second. So the first commit can
  never conflict on an upstream merge and the second is the whole conflict
  surface. A merged PR leaves a third commit behind — fold it back in (it is
  a rewrite of a published branch: Jan force-pushes and bumps the chumby-pi
  gitlink, not you).
