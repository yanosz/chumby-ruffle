...
| Gap | Note |
|-----|------|
| `/tmp` volatility | Real hardware's `/tmp` is a ramdisk, wiped per boot; the virtual rootfs persists it. The one observable consequence (stale `/tmp/musicsource` → inert resume PLAY) is fixed pointwise — the host deletes that file at start. Clearing all of `/tmp` at start would be faithful, but collides with the boot machinery: committed fixtures live there (`currentProfileID/Name` among them), and the appliance launcher writes the local-widgets `profile.xml` overlay there before the player starts. A session of its own, if ever. |
