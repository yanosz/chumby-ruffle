...
| Boot-time intro | The in-panel half shipped 2026-07-11: the INTRO button plays `intro.swf` on the localCache path via prototype replacement (`chumby/intro.rs`, design §3), and the `enable_intro`/`disable_intro` backticks really toggle `/psp/disable_intro` in the rootfs. What remains is the *boot* entry point: real hardware's `rcS` runs `start_intro` — a standalone player on `intro.swf`, every boot until `/psp/disable_intro` exists — *before* the panel. Undecided (Jan): a pre-panel player invocation in the appliance launcher (faithful, chumby-pi side) vs. skipping it. Player-side readiness is designed but **not fully verified**: standalone `intro.swf` was seen playing (that is how the no-audio freeze was bisected), but the standalone *ending* — exit → "control" screen → the enable/disable buttons writing the flag → `fscommand("quit")` actually exiting the process (the swallow is gated on `g_playingIntro`, which standalone never sets) — has not been exercised, and neither has replaying the intro twice in one panel session. First items for the session that wires the boot entry. |
